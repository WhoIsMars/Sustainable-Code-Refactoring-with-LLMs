...
def analyze_base_entries(
    target_languages=["cpp", "c", "go", "typescript", "python", "javascript", "java"],
    #default all languages 
):
    """Analyze base entries with 100% pass rate."""

    execution_outputs_dir = utility_paths.OUTPUT_DIR_FILEPATH
    ...
    for file in execution_outputs_dir.glob("*_results_*.json"):
        ...
        if (
            "_v1_" in file.name
            or "_v2_" in file.name
            or "_v3_" in file.name
            or "_v4_" in file.name
        ):
            continue
        if general_utils.skip_pattern_is_present(file.name):
            continue
    ...
    cluster_executions = defaultdict(
        lambda: defaultdict(list)
    )  # cluster -> language -> [execution_data]
    ...
    for cluster_name, languages_data in cluster_executions.items():
        for language, entries in languages_data.items():
            ...
            for entry_id in unique_ids:
                ...
                passed = sum(
                    1 for e in entry_executions if e.get("regressionTestPassed", False)
                )
    ...
    for lang in target_languages:
        ...
        print(f"  {'=' * 70}")
        print(
            f"  100% pass rate (5/5):  {s['valid_entries_100']:>3} ({s['valid_entries_100'] / total * 100:>5.1f}%)"
        )
        print(
            f"  ≥80% pass rate (4-5/5): {s['valid_entries_80']:>3} ({s['valid_entries_80'] / total * 100:>5.1f}%)"
        )
        print(
            f"  ≥60% pass rate (3-5/5): {s['valid_entries_60']:>3} ({s['valid_entries_60'] / total * 100:>5.1f}%)"
        )
        print(
            f"  <60% pass rate:         {s['failed_entries']:>3} ({s['failed_entries'] / total * 100:>5.1f}%)"
        )
    ...
    report_file =  utility_paths.ANALISYS_REPORT_FILE_PATH / "base_entries_status.json"
