...
def format_entries(currency, locale, entries):
    if currency == "USD":
        ...
        symbol = currency  # Handle other currencies gracefully
    ...
    for date, description, change in sorted(entries):
        ...
        change_abs_list = list(change_abs)
        i = len(change_abs_list) - 6
        while i > 0:
            change_abs_list.insert(i, thousands)
            i -= 3
        change_abs = "".join(change_abs_list).replace(".", decimal)
