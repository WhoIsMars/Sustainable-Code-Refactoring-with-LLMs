...
def format_entries(currency, locale, entries):
    ...
    for date, description, change in sorted(entries):
        formatted_date = date_fmt.format(*date)
        truncated_description = description[:22] + "... " if len(description) > 25 else description
        change_abs = f"{abs(change) / 100:,.2f}".replace(",", "TEMP").replace(".", decimal).replace("TEMP", thousands)
        change_formatted = (number_ne_fmt if change < 0 else number_po_fmt).format(symbol, change_abs)

        ret.append("{:<11}| {:<26}| {:>13}".format(formatted_date, truncated_description, change_formatted))
