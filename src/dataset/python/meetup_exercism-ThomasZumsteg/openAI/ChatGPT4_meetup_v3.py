...
a predefined natural language description. e.g. 1st, 2nd, last, teenth
"""
...
def meetup_day(year, month, weekday, condition):
    ...
    days_in_month = (datetime.date(year, month + 1, 1) - first_day).days if month < 12 else 31

    for day in range(1, days_in_month + 1):
        current_day = datetime.date(year, month, day)
        if current_day.weekday() == weekday_num and condition_func(current_day):
            return current_day
...
def teenth(day):
    """True on a days ending in -teenth, i.e., seventeenth"""
