...
def is_rect(corners, diagram):
    ...
    for i in range(a_c + 1, d_c):
        if diagram[a_r][i] != '-' and diagram[a_r][i] != '+':
            return False
        if diagram[d_r][i] != '-' and diagram[d_r][i] != '+':
            return False

    for i in range(a_r + 1, d_r):
        if diagram[i][a_c] != '|' and diagram[i][a_c] != '+':
            return False
        if diagram[i][d_c] != '|' and diagram[i][d_c] != '+':
            return False
