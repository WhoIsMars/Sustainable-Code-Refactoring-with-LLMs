...
def is_rect(corners, diagram):
    ...
    if a_r != b_r or c_r != d_r or a_c != c_c or b_c != d_c:
        return False

    top = diagram[a_r][a_c:d_c + 1]
    bottom = diagram[c_r][a_c:d_c + 1]
    left = ''.join(row[a_c] for row in diagram[a_r:c_r + 1])
    right = ''.join(row[d_c] for row in diagram[a_r:c_r + 1])

    horz_pattern = f'+{"-" * (d_c - a_c - 1)}+'
    vert_pattern = f'+{"|" * (c_r - a_r - 1)}+'

    return re.fullmatch(horz_pattern, top) and re.fullmatch(horz_pattern, bottom) and \
           re.fullmatch(vert_pattern, left) and re.fullmatch(vert_pattern, right)