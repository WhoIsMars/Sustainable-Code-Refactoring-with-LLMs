class BowlingGame:
    ...
    def roll(self, pins):
        ...
        if self._frame % 2 == 1 and self._rolls[-1] + pins > 10:
    ...
    def score(self):
        ...
        for frame in range(10):
            if self._rolls[index] == 10:  # Strike
                total += 10 + sum(self._rolls[index + 1:index + 3])
                index += 1
    ...
    def _update_state(self, pins):
        if self._frame < 18:  # Regular frames
            ...
            self._frame += 1
            if pins == 10 or sum(self._rolls[-2:]) == 10:
                self._extra_rolls = 2 if pins == 10 else 1
