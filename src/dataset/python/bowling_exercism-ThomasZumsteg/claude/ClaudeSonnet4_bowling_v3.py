class BowlingGame(object):
    ...
    def _update_state(self):
        last_roll = self._rolls[-1]
        is_strike = last_roll == 10

        self._frame = min(20, self._frame + (2 if is_strike else 1))
        self._first = True if is_strike else not self._first

        if self._frame == 20 and self._extra_rolls is None:
            if is_strike:
                self._extra_rolls = 2
            elif len(self._rolls) >= 2 and self._rolls[-2] + last_roll == 10:
                self._extra_rolls = 1
            else:
                self._extra_rolls = 0
