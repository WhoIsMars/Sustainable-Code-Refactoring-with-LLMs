export default class {
  ...
  resetName(): void {
    ...
    while (newName === null && retries < ClassNameGenerator.MAX_RETRIES) {
    ...
    if (newName === null) {
      console.warn("Failed to generate a unique name after maximum retries. Generating a fallback name.");
  ...
  private generateRandomName(): string {
    let newRandomName = "";
    for (let i = 0; i < 2; i++) {
      newRandomName += ClassNameGenerator.LETTERS[Math.floor(Math.random() * ClassNameGenerator.LETTERS.length)];
    }
    for (let i = 0; i < 3; i++) {
