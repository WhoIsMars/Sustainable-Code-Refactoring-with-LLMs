class SimpleCipher {
  ...
  encode(message: string): string {
    ...
    for (let i = 0; i < message.length; i++) {
      ...
      const adjustment = charCode >= 65 && charCode <= 90 ? charCode - 65 : charCode - 97;
      const keyAdjustment = this.key.charCodeAt(i % keyLength) - 97;
      encryptedMessage += String.fromCharCode(((adjustment + keyAdjustment) % SimpleCipher.ALPHABET_LENGTH) + 97);
    }
  ...
  decode(encryptedMessage: string): string {
    ...
    for (let i = 0; i < encryptedMessage.length; i++) {
      ...
      const keyAdjustment = this.key.charCodeAt(i % keyLength) - 97;
      decryptedMessage += String.fromCharCode(((adjustment - keyAdjustment + SimpleCipher.ALPHABET_LENGTH) % SimpleCipher.ALPHABET_LENGTH) + 97);
    }
