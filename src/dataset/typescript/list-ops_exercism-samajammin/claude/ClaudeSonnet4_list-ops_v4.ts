export default class List<T> {
  ...
  public concat(list: List<any>): List<any> {
    const result = new List<any>(this.values);
  ...
  public reverse(): List<{}> {
    const reversed: {}[] = new Array(this.values.length);
    for (let i = 0; i < this.values.length; i++) {
      reversed[i] = this.values[this.values.length - 1 - i];
    }
    const result = new List();
    result.values = reversed;
    result._length = reversed.length;
    return result;
  }
