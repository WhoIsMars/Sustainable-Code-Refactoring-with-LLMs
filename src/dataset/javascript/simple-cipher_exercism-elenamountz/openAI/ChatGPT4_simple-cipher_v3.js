...
export class Cipher {
  ...
  encode(str) {
    const extendedKey = this._extendKey(str.length);
    return Array.from(str, (char, i) => 
      abc[(abc.indexOf(char) + abc.indexOf(extendedKey[i])) % LENGTH]
    ).join('');
  }
  ...
  decode(str) {
    const extendedKey = this._extendKey(str.length);
    return Array.from(str, (char, i) => 
      abc[(abc.indexOf(char) - abc.indexOf(extendedKey[i]) + LENGTH) % LENGTH]
    ).join('');
  }
