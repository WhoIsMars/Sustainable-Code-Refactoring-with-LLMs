class Bowling {
    ...
    roll(pins) {
        ...
        if (pins < 0 || pins > 10) throw new Error('Invalid pin count');
        if (this.second && this.rolls[this.rolls.length - 1] + pins > 10) throw new Error('Pin count exceeds pins on the lane');

        if (this.frame === 9) {
            if (!this.second && pins === 10) this.extra = 2;
            else if (this.second && this.rolls[this.rolls.length - 1] + pins === 10) this.extra = 1;
