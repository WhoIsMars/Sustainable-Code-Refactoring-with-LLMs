...
export class Song {
  ...
  verse(index) {
    ...
    let verse = `I know an old lady who swallowed a ${verseData.type}.\n${verseData.message}`;
    let i = index - 1;
    ...
    while (i > 0) {
      ...
      verse += `She swallowed the ${v.type} to catch the ${v.catch}.\n`;
    }

    const firstVerse = this._verses[0];
    verse += `I don't know why she swallowed the ${firstVerse.type}.${firstVerse.end ? firstVerse.end : ''}`;
