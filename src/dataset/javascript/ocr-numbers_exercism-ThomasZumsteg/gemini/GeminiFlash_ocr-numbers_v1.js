...
function lines_to_characters(OCR_lines, startLine) {
    const line = OCR_lines;
    const characters = [];
    const lineLength = line[startLine].length;
    const numChars = lineLength / 3;

    for (let c = 0; c < numChars; c++) {
        let characterLines = [];
        for (let i = 0; i < 4; i++) {
            characterLines.push(line[startLine + i].slice(c * 3, c * 3 + 3));
        }
        characters.push(characterLines.join("\n"));
    }
    return characters;
}
