...
// Pre-compute static strings to avoid repeated concatenation
var lines = {
...
// Pre-compute reversed chorus segments to avoid repeated operations
var reversedChorus = [];
for (var i = 0; i < lines.chorus.length; i++) {
	reversedChorus[i] = lines.chorus.slice(0, i + 1).reverse().join('');
}

FoodChain.prototype.verse = function( verseNum ) {
	var verse = lines.opening + lines.first[verseNum];
	if (verseNum >= 8) { return verse; }
	return verse + reversedChorus[verseNum - 1];
};
