...
class TwelveDays {
    verse(verses) {
        ...
        const end = verses[1] === undefined ? start + 1 : verses[1];
        const result = [];

        for (let current = start; current < end; current++) {
            let line = `On the ${days[current].day} day of Christmas my true love gave to me`;
            for (let v = current; v >= 0; v--) {
                line += `, ${v === 0 && current !== 0 ? "and " : ""}${days[v].gift}`;
            }
            result.push(line + ".\n");
        }
        return result.join('\n');
