...
const DAY_PREFIX = "On the ";
const DAY_SUFFIX = " day of Christmas my true love gave to me";
const VERSE_ENDING = ".\n";
...
class TwelveDays {
    verse(verses) {
        ...
        for (let current = start; current < end; current++) {
            const parts = [DAY_PREFIX + days[current].day + DAY_SUFFIX];
            
            for (let v = current; v >= 0; v--) {
                const prefix = v === 0 && current !== 0 ? ", and " : ", ";
                parts.push(prefix + days[v].gift);
            }
            
            result.push(parts.join("") + VERSE_ENDING);
        }
