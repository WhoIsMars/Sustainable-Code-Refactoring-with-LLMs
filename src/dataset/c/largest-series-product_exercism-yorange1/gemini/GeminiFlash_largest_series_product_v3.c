...
int64_t largest_series_product(char *digits, size_t span) {
    ...
    for (size_t i = span; i < len; ++i) {
        ...
        if (!isdigit(digits[i - span])) {
            return -1;
        }

        int64_t removed_digit = digits[i - span] - '0';
        int64_t added_digit = digits[i] - '0';

        if (removed_digit != 0) {
            current_product /= removed_digit;
        } else {
            current_product = 1;
