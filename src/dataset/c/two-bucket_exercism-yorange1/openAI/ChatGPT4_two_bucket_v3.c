...
static void pour(bucket_id_t from, bucket_id_t to)
{
    ...
    printf("pouring from bucket %d to %d\n", from, to);
}
                        ...
                        bucket_liters_t goal_volume, bucket_id_t start_bucket)
{
    ...
    {
        result.goal_bucket = (bucket_1_size == goal_volume) ? BUCKET_ID_1 : BUCKET_ID_2;
        result.move_count = (start_bucket == result.goal_bucket) ? 1 : 2;
        result.other_bucket_liters = (start_bucket == result.goal_bucket) ? 0 : 
                                      (result.goal_bucket == BUCKET_ID_1 ? bucket_2_size : bucket_1_size);
        return result;
    }
