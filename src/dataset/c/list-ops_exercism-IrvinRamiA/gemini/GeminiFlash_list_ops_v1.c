...
list_t *new_list(size_t length, list_element_t elements[]) {
  ...
  if (length > 0 && elements != NULL) {
    ...
    list->elements = NULL; // Ensure elements is NULL for empty lists
  }
...
list_t *append_list(list_t *list1, list_t *list2) {
  ...
  if (list1 == NULL) {
    list_t *new_list_ptr = new_list(list2->length, list2->elements);
    return new_list_ptr;
  }

  if (list2 == NULL) {
    list_t *new_list_ptr = new_list(list1->length, list1->elements);
    return new_list_ptr;
  }
...
list_t *filter_list(list_t *list, bool (*filter)(list_element_t)) {
  ...
  list_element_t *filtered_elements =
      (list_element_t *)malloc(list->length * sizeof(list_element_t)); // Max size

  if (filtered_elements == NULL) {
    return new_list(0, NULL); // Handle allocation failure
  }
  ...
  free(filtered_elements); // Free the over-allocated memory
...
list_t *map_list(list_t *list, list_element_t (*map)(list_element_t)) {
  ...
  if (mapped_list->elements == NULL && list->length > 0) {
    mapped_list->elements = (list_element_t *)malloc(list->length * sizeof(list_element_t));
...
list_element_t foldl_list(list_t *list, list_element_t initial,
  ...
  for (size_t i = 0; i < list->length; i++) {
    result = foldl(result, list->elements[i]); // Correct order for foldl
  }
  return result;
}
...
list_t *reverse_list(list_t *list) {
  ...
  if (reversed_list->elements == NULL && list->length > 0) {
    reversed_list->elements = (list_element_t *)malloc(list->length * sizeof(list_element_t));
