...
list_t *append_list(list_t *list1, list_t *list2)
{
    ...
    list_t *list = (list_t *)malloc(sizeof(list_t) + total_length * sizeof(list_element_t));
    if (!list) return NULL;
...
{
    if (!list || !filter) return new_list(0, NULL);

    list_element_t *temp_elements = (list_element_t *)malloc(list->length * sizeof(list_element_t));
    if (!temp_elements) return new_list(0, NULL);

    size_t filtered_count = 0;
    for (size_t i = 0; i < list->length; i++) {
        if (filter(list->elements[i])) {
            temp_elements[filtered_count++] = list->elements[i];
        }
    }

    list_t *filtered_list = new_list(filtered_count, temp_elements);
    free(temp_elements);
