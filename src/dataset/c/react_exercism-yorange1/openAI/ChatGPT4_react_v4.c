...
struct cell *create_compute1_cell(struct reactor *reactor, struct cell *input_cell, compute1 compute)
{
    ...
    struct cell *compute_cell = calloc(1, sizeof(struct cell));
...
int get_cell_value(const struct cell *cell)
{
    assert(cell != NULL);
    return cell->value;
}
...
static void update_value(struct cell *cell)
{
    if (cell->type != INPUT)
    {
        ...
        if (compute_value(cell) == prev_value)
        {
            return;
        }
    }
...
callback_id add_callback(struct cell *compute_cell, void *info, callback func)
{
    ...
    {
        struct compute_callback *callback = &compute_cell->callbacks[id];
        if (!callback->func)
        {
            callback->cell_value = compute_cell->value;
            callback->func = func;
            callback->info = info;
            return id;
        }
