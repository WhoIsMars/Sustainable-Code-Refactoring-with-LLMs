...
static inline bool add_output_cell(struct cell *cell, struct cell *output)
{
...
struct cell *create_compute1_cell(struct reactor *reactor, struct cell *input_cell, compute1 compute)
{
    ...
    bool added = add_output_cell(input_cell, compute_cell);
    assert(added);
                                  ...
                                  struct cell *input_cell2, compute2 compute)
{
    ...
    bool added1 = add_output_cell(input_cell1, compute_cell);
    bool added2 = add_output_cell(input_cell2, compute_cell);
...
static inline int compute_value(struct cell *cell)
{
    switch (cell->type) {
        case COMPUTE1: {
            ...
            int new_value = func(cell->inputs[0]->value);
            cell->value = new_value;
            return new_value;
        }
        case COMPUTE2: {
            compute2 func = cell->compute.func2;
            int new_value = func(cell->inputs[0]->value, cell->inputs[1]->value);
            cell->value = new_value;
            return new_value;
        }
        default:
...
static void check_callbacks(struct cell *cell)
{
    if (cell->type != INPUT) {
        for (size_t i = 0; i < MAX_CELL_CALLBACKS_NUM; i++) {
            ...
            if (callback->func != NULL && cell->value != callback->cell_value) {
