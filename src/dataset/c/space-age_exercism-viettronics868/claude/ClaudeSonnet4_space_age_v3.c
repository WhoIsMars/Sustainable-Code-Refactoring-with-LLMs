...
static const float SECONDS_TO_YEARS = 1.0f / (365.25f * 24.0f * 3600.0f);

float seconds_to_Earth_year(uint64_t seconds){
    return seconds * SECONDS_TO_YEARS;
}
...
float age(planet_t planet, uint64_t seconds){
    float planet_ratio = planet_to_Earth_year(planet);
    if (planet_ratio == -1.0f) return -1.0f;
    return seconds_to_Earth_year(seconds) / planet_ratio;
}