...
static void count_letter(const char *letters, size_t length, uint8_t alpha_table[ALPHA_NUM])
{
    ...
    {
        size_t index = (unsigned char)letters[i] | 0x20;  // Fast lowercase conversion
        if (index >= 'a' && index <= 'z') {
            alpha_table[index - 'a']++;
        }
    }
...
static bool is_anagrams(size_t subject_length, const uint8_t alpha_table[ALPHA_NUM], const char *word, size_t word_length)
{
    ...
    {
        size_t index = (unsigned char)word[i] | 0x20;  // Fast lowercase conversion
        if (index < 'a' || index > 'z' || tmp_table[index - 'a'] == 0)
        {
            return false;
        }
        tmp_table[index - 'a']--;
