...
static inline void count_letter(const char *letters, size_t length, uint8_t alpha_table[ALPHA_NUM])
{
    ...
    for (size_t i = 0; i < length; i++)
    {
        ...
        {
            alpha_table[tolower(letters[i]) - 'a']++;
        }
    }
}

static inline bool is_anagrams(size_t subject_length, const uint8_t alpha_table[ALPHA_NUM], const char *word)
{
    ...
    {
        if (!isalpha(word[i]) || tmp_table[tolower(word[i]) - 'a']-- == 0)
        {
            return false;
        }
    }
...
void find_anagrams(const char *subject, struct candidates *candidates)
{
    ...
    for (size_t i = 0; i < candidates->count; i++)
    {
        ...
        candidate->is_anagram = (strcasecmp(subject, candidate->word) != 0 &&
