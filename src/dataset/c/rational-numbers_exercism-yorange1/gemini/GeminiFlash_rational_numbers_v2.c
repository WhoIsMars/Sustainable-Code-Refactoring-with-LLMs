...
rational_t reduce(rational_t r) {
    ...
    uint16_t gcd_val = gcd((uint16_t)num, (uint16_t)den);

    rational_t ret;
    ret.numerator = num / gcd_val;
    ret.denominator = den / gcd_val;

    if (num_neg != den_neg && ret.numerator != 0) {
...
float exp_real(uint16_t x, rational_t r) {
    ...
    if (exponent > 0) {
        ...
        result = 1.0f / powf(base, -exponent);
    }
    return result;
}