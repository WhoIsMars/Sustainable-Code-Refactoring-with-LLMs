...
static unsigned int weekday_to_num(const char *weekday)
{
    for (unsigned int i = 0; i < 7; i++) {
        if (!strcmp(weekdays + weekday_offsets[i], weekday))
            return i;
    }
    return 8;
}
                        ...
                        const char *day_of_week)
{
    ...
    char first_char = week[0];
    switch (first_char) {
        case 'f': // first
            return day;
        case 's': // second
            return day + 7;
        case 't': // third or teenth
            return week[1] == 'h' ? day + 14 : 
                   ((day + 7) < 13 ? day + 14 : day + 7);
        case 'f': // fourth (handled by first case)
            return day + 21;
        case 'l': // last
            {
        ...
        default:
            if (first_char == 'f' && week[1] == 'o') // fourth
                return day + 21;
            return -1;
    }
}