...
static unsigned char weekday_to_num(const char *weekday)
{
...
static inline unsigned char is_leap(unsigned int year)
{
                        ...
                        const char *day_of_week)
{
    ...
    unsigned char wday = weekday_to_num(day_of_week);
    if (wday > 7) return -1;

    unsigned char day = 1 + wday - tm.tm_wday;
    if (day <= 0) day += 7;
    ...
    switch (week_first) {
        ...
        case 'f' + 1:
            return day + 21;
        case 'l': {
            unsigned char ndays = days_in_month[is_leap(year)][month - 1];
