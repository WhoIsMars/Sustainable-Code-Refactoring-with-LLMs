...
static inline unsigned int weekday_to_num(const char *weekday) {
...
int meetup_day_of_month(unsigned int year,
    ...
    unsigned int wday = weekday_to_num(day_of_week);
    if (wday > 7) return -1;
    ...
    int first_wday_offset = (wday - tm.tm_wday + 7) % 7;
    int day = 1 + first_wday_offset;
