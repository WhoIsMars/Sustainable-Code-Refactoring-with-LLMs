...
saddle_points_t *saddle_points(uint8_t rows, uint8_t cols, uint8_t matrix[rows][cols]) {
    ...
    for (uint8_t i = 0; i < rows; i++) {
        for (uint8_t j = 0; j < cols; j++) {
            ...
            // Check if current element is greater than or equal to all other elements in the same row
            ...
            // If it's still a potential saddle point, check if it's less than or equal to all other elements in the same column
            if (is_saddle_point) {
            ...
            // If it's a saddle point, add it to the array
            if (is_saddle_point) {
    ...
    // Copy the saddle points to the result array
