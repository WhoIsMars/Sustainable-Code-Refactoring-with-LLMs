...
char *to_roman_numeral(unsigned int number) {
  // Allocate enough memory for the largest possible Roman numeral (3999 -> "MMMCMXCIX")
  char *output = (char *)malloc(16 * sizeof(char));
  if (!output) return NULL; // Ensure memory allocation succeeded
  char *ptr = output;

  for (int i = 0; i < 13 && number > 0; i++) {
    while (number >= arabicToRomanMap[i].arabic) {
      size_t len = strlen(arabicToRomanMap[i].roman);
      memcpy(ptr, arabicToRomanMap[i].roman, len);
      ptr += len;
