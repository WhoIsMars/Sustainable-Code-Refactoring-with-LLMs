...
resistor_value_t color_code(const resistor_band_t colors[3])
{
    ...
    // Precalculate the multiplier to avoid using pow() which is computationally expensive.
    switch (colors[2]) {
        case 0: break;
        ...
        default: break; // Handle potential errors, though resistor bands are typically limited.
    }
