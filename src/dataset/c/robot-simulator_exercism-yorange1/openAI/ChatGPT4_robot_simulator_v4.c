...
void robot_move(robot_status_t *robot, const char *commands)
{
    ...
    {
        if (*commands == 'R') robot->direction = (robot_direction_t)((robot->direction + 1) % DIRECTION_MAX);
        else if (*commands == 'L') robot->direction = (robot_direction_t)((robot->direction + 3) % DIRECTION_MAX);
        else if (*commands == 'A') robot_advance(robot);
    }
}