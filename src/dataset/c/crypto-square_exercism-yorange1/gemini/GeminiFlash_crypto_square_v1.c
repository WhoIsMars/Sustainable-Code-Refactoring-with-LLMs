...
char *ciphertext(const char *input) {
  ...
  char *normalized = malloc(input_len + 1); // Allocate maximum possible size
  if (normalized == NULL) {
    return NULL; // Handle allocation failure
  }
  ...
  for (size_t i = 0; i < input_len; i++) {
    if (isalnum(input[i])) {
      normalized[normalized_len] = tolower(input[i]);
      normalized_len++;
    }
  }
  ...
  char *cipher = malloc(rows * cols + rows); // Allocate for ciphertext + spaces
  if (cipher == NULL) {
    free(normalized);
    return NULL; // Handle allocation failure
  }
