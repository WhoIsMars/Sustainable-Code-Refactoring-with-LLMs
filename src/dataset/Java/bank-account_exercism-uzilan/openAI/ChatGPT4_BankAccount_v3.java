public class BankAccount {
    ...
    public int getBalance() throws BankAccountActionInvalidException {
        if (!open) throw new BankAccountActionInvalidException(ACCOUNT_CLOSED);
        return account;
    }
    ...
    public synchronized void withdraw(int amount) throws BankAccountActionInvalidException {
        if (!open) throw new BankAccountActionInvalidException(ACCOUNT_CLOSED);
        if (amount < 0) throw new BankAccountActionInvalidException(NEGATIVE_AMOUNT);
        if (account == 0) throw new BankAccountActionInvalidException(EMPTY_ACCOUNT);
        if (account < amount) throw new BankAccountActionInvalidException(WITHDRAWING_TOO_MUCH);
        account -= amount;
    }
}