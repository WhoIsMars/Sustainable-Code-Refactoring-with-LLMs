...
class DifferenceOfSquaresCalculator {
    ...
    public static BigInteger computeSumOfSquaresTo(BigInteger b) {
        return b.multiply(b.add(BigInteger.ONE).multiply(BigInteger.TWO.multiply(b).add(BigInteger.ONE))).divide(BigInteger.valueOf(6));
    }
    ...
    public static BigInteger computeDifferenceOfSquares(BigInteger b) {
        BigInteger squareOfSum = b.multiply(b.add(BigInteger.ONE)).divide(BigInteger.TWO);
        squareOfSum = squareOfSum.multiply(squareOfSum);
        BigInteger sumOfSquares = b.multiply(b.add(BigInteger.ONE).multiply(BigInteger.TWO.multiply(b).add(BigInteger.ONE))).divide(BigInteger.valueOf(6));
    ...
    static void findMaxima() {
        ...
        while (lastResult > 0) {
            i++;
            int currentResult = computeSquareOfSumTo(i);
            if (currentResult <= 0 && lastResult > 0) {
        ...
        while (lastResult > 0) {
            i++;
            int currentResult = computeSumOfSquaresTo(i);
            if (currentResult <= 0 && lastResult > 0) {
                System.out.println("An overflow happened at " + i);
        ...
        while (lastResult > 0) {
            i++;
            int currentResult = computeDifferenceOfSquares(i);
            if (currentResult <= 0 && lastResult > 0) {
                System.out.println("An overflow happened at " + i);
