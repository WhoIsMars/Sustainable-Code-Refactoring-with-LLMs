namespace zhang {
    constexpr int zhang_part{8'541};
    constexpr int red_code{512};
    constexpr int blue_code{677};

    int bank_number_part(int secret_modifier) {
        return (zhang_part * secret_modifier) % 10000;
    }
    namespace red {
        constexpr int code_fragment() { return red_code; }
    }
    namespace blue {
        constexpr int code_fragment() { return blue_code; }
    }
}

namespace khan {
    constexpr int khan_part{4'142};
    constexpr int red_code{148};
    constexpr int blue_code{875};

    int bank_number_part(int secret_modifier) {
        return (khan_part * secret_modifier) % 10000;
    }
    namespace red {
        constexpr int code_fragment() { return red_code; }
    }
    namespace blue {
        constexpr int code_fragment() { return blue_code; }
    }
}

namespace garcia {
    constexpr int garcia_part{4'023};
    constexpr int red_code{118};
    constexpr int blue_code{923};

    int bank_number_part(int secret_modifier) {
        return (garcia_part * secret_modifier) % 10000;
    }
    namespace red {
        constexpr int code_fragment() { return red_code; }
    }
    namespace blue {
        constexpr int code_fragment() { return blue_code; }
    }
}

namespace estate_executor {
    int assemble_account_number(int secret_modifier) {
        return zhang::bank_number_part(secret_modifier) +
               khan::bank_number_part(secret_modifier) +
               garcia::bank_number_part(secret_modifier);
    }

    int assemble_code() {
        constexpr int total_blues = zhang::blue::code_fragment() + khan::blue::code_fragment() + garcia::blue::code_fragment();
        constexpr int total_reds = zhang::red::code_fragment() + khan::red::code_fragment() + garcia::red::code_fragment();
        return total_blues * total_reds;
    }
}