...
Complex Complex::operator/(Complex const &obj) const {
  const double norm_squared = obj.real() * obj.real() + obj.imag() * obj.imag();
  const double real_part = (real_ * obj.real() + imag_ * obj.imag()) / norm_squared;
  const double imag_part = (imag_ * obj.real() - real_ * obj.imag()) / norm_squared;
