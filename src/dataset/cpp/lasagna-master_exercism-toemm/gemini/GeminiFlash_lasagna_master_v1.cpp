...
std::vector<double> scaleRecipe(const std::vector<double>& quantities, int nr) {
    ...
    double scale_factor = static_cast<double>(nr) / 2.0;

    std::transform(quantities.begin(), quantities.end(), new_quantities.begin(), 
                   [scale_factor](double quantity) { return quantity * scale_factor; });
