...
date_independent::clock::operator std::string() const {
  int hour = minutes / MINUTES_HOUR;
  int minute = minutes % MINUTES_HOUR;

  std::string hour_str = (hour < 10 ? "0" : "") + std::to_string(hour);
  std::string minute_str = (minute < 10 ? "0" : "") + std::to_string(minute);

  return hour_str + ":" + minute_str;
}

int date_independent::clock::normalize_minutes(int ms) {
  ms %= MINUTES_DAY;
  if (ms < 0) {
    ms += MINUTES_DAY;
  }
