namespace squares {
    ...
    {
        unsigned long long int n_ull = static_cast<unsigned long long int>(n);
        return n_ull * (n_ull + 1) * (2 * n_ull + 1) / 6;
    }

    unsigned long long int difference(const int n)
    {
        unsigned long long int square_of_sum_val = square_of_sum(n);
        unsigned long long int sum_of_squares_val = sum_of_squares(n);
