...
heaven::Vessel::Vessel(std::string name, int i, star_map::System sys) : name(std::move(name)), generation(i), current_system(sys), busters(0) {}

heaven::Vessel heaven::Vessel::replicate(std::string newname) {
    return Vessel {std::move(newname), this->generation+1, star_map::System::Sol};
}
