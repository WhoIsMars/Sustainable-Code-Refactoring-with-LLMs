...
cipher::cipher(std::string text) {
  // Remove non-alphanumeric characters and convert to lowercase in one pass
  ...
  // Reserve capacity for segmented_text
  segmented_text.reserve((this->text.size() + column_length - 1) / column_length);
...
std::string cipher::normalized_cipher_text() {
  std::string result;
  // Estimate capacity: text size + spaces between columns + padding spaces
  result.reserve(text.size() + column_length * segmented_text.size());
