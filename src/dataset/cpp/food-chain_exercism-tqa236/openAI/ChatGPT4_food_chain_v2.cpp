...
std::string food_chain::verse(int num) {
    ...
    res << "I know an old lady who swallowed a " << song_info[num].animal << ".\n";

    if (num > FLY) {
        res << song_info[num].reaction << "\n";
    }

    if (num == HORSE) return res.str();

    while (num > FLY) {
        res << "She swallowed the " << song_info[num].animal << " to catch the " << song_info[num - 1].animal;
        if (--num == SPIDER) {
