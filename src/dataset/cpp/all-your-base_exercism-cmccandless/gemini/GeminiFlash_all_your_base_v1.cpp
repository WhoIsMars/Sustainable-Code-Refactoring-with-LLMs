...
std::vector<unsigned int> fromDecimalNumber(unsigned int decimalNumber,
  ...
  digits.reserve(32); // Reserve space to avoid reallocations
...
std::vector<unsigned int> convert(unsigned int fromBase,
  ...
  unsigned int decimalNumber = toDecimalNumber(fromBase, digits);
  return fromDecimalNumber(decimalNumber, toBase);
}
