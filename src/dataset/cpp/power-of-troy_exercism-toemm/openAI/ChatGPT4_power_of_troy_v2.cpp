...
human::human() : influenced_by(nullptr), possession(nullptr), own_power(nullptr) {}
...
void manifest_power(human& hum, const std::string& pwr) {
    if (!hum.own_power || hum.own_power.unique()) {
        hum.own_power = std::make_shared<power>(pwr);
    } else {
        hum.own_power = std::make_shared<power>(*hum.own_power);
    }
}
