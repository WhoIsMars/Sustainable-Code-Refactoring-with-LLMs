...
std::vector<int> round_down_scores(std::vector<double> student_scores) {
    ...
    rounded_down_scores.reserve(student_scores.size()); // Pre-allocate memory

    std::transform(student_scores.begin(), student_scores.end(), std::back_inserter(rounded_down_scores),
...
std::vector<std::string> student_ranking(
    ...
    for (size_t i = 0; i < student_scores.size(); ++i) {
        results.emplace_back(std::to_string(i + 1) + ". " + student_names[i] + ": " + std::to_string(student_scores[i]));
        std::cout << results.back() << std::endl; // Print immediately after creation
    }
