#include "queen_attack.h"

namespace queen_attack {
chess_board::chess_board() = default;

chess_board::chess_board(Position white, Position black)
    : white_position{white}, black_position{black} {
  if (white_position == black_position) {
    throw domain_error("Queen positions must be distinct.");
  }
}

Position chess_board::white() const noexcept { return white_position; }

Position chess_board::black() const noexcept { return black_position; }

bool chess_board::can_attack() const noexcept {
  const int col_diff = white_position.first - black_position.first;
  const int row_diff = white_position.second - black_position.second;
  return col_diff == 0 || row_diff == 0 || 
         ((col_diff ^ row_diff) == 0) || ((col_diff ^ -row_diff) == 0);
}

unsigned chess_board::get_queen_position_in_string(Position coordinates) const noexcept {
  return (coordinates.first << 4) + (coordinates.second << 1);
}

chess_board::operator string() const {
  static constexpr char board_template[] = 
      "_ _ _ _ _ _ _ _\n"
      "_ _ _ _ _ _ _ _\n"
      "_ _ _ _ _ _ _ _\n"
      "_ _ _ _ _ _ _ _\n"
      "_ _ _ _ _ _ _ _\n"
      "_ _ _ _ _ _ _ _\n"
      "_ _ _ _ _ _ _ _\n"
      "_ _ _ _ _ _ _ _\n";
  
  string board{board_template, sizeof(board_template) - 1};
  
  const auto white_pos = get_queen_position_in_string(white_position);
  const auto black_pos = get_queen_position_in_string(black_position);

  board[white_pos] = 'W';
  board[black_pos] = 'B';

  return board;
}
}  // namespace queen_attack