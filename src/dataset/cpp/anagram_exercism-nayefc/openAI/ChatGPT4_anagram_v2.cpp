...
std::vector<std::string> anagram::anagram::matches(const std::vector<std::string>& match_list) {
    ...
    for (const auto& match_word : match_list) {
        if (match_word.size() == this->word.size() && !std::equal(match_word.begin(), match_word.end(), this->word.begin(), this->word.end(), [](char a, char b) { return tolower(a) == tolower(b); }) && this->is_anagram(match_word)) {
