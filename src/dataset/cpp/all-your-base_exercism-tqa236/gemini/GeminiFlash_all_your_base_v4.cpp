...
vector<unsigned int> convert(int in_base, vector<unsigned int> in_digits,
  ...
  if (std::any_of(in_digits.begin(), in_digits.end(),
                  [in_base](unsigned int digit) { return digit >= (unsigned int)in_base; })) {
    throw invalid_argument("Invalid base.");
  }

  unsigned int dec_num = 0;
  for (unsigned int digit : in_digits) {
    if (dec_num > UINT_MAX / in_base || digit > UINT_MAX - dec_num * in_base) {
      throw invalid_argument("Input digits too large for unsigned int.");
    }
