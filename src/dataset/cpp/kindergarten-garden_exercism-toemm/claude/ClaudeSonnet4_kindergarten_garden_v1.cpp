...
std::array<Plants, 4> plants(std::string_view flowers, std::string_view student) {
    ...
    const int second_row_offset = flowers.size() / 2;
    
    return {
        static_cast<Plants>(flowers[students_index]),
        static_cast<Plants>(flowers[students_index + 1]),
        static_cast<Plants>(flowers[second_row_offset + students_index]),
        static_cast<Plants>(flowers[second_row_offset + students_index + 1])
    };
}
