...
string run_length_encoding::encode(const string &decoded) noexcept {
    ...
    string encoded;
    encoded.reserve(decoded.size()); // Reserve space to avoid reallocations
...
string run_length_encoding::decode(const string &encoded) noexcept {
    ...
    string decoded;
    decoded.reserve(encoded.size() * 2); // Conservative estimate to reduce reallocations
    ...
    for (char ch : encoded) {
        if (ch >= '0' && ch <= '9') {
            ...
            if (count == 0) count = 1;
            decoded.append(count, ch); // More efficient than loop
            count = 0;
        }
    }
    return decoded;
