...
boost::gregorian::date scheduler::teenth_day(boost::date_time::weekdays const& d) const {
  ...
  boost::gregorian::weekday current_weekday = date.day_of_week();
  int days_to_add = (d - current_weekday + 7) % 7;
...
boost::gregorian::date scheduler::first_day(boost::date_time::weekdays const& d) const {
  ...
  boost::gregorian::weekday first_weekday = first_of_month.day_of_week();
  int days_to_add = (d - first_weekday + 7) % 7;
  return first_of_month + boost::gregorian::days(days_to_add);
}
...
boost::gregorian::date scheduler::last_day(boost::date_time::weekdays const& d) const {
  boost::gregorian::date last_day_of_month(year_, month_, boost::gregorian::day(boost::gregorian::date(year_, month_ + 1, 1) - boost::gregorian::days(1)));
  boost::gregorian::weekday last_weekday = last_day_of_month.day_of_week();
  int days_to_subtract = (last_weekday - d + 7) % 7;
