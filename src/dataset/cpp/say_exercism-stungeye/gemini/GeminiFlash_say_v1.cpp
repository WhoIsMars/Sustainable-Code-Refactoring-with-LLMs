   ...
   int largest_named_divisor(const long long number) {
        ...
        for (const auto& pair : named_place_values) {
            if (number >= pair.first) {
                ...
                break; // Optimization: Stop once we find a value larger than the number
            }
        }
        return divisor;
    }
    ...
    std::string in_english_100_and_greater(const long long number) {
        ...
        std::string result = in_english(quotient) + " " + [&]() -> std::string {
            for (const auto& pair : named_place_values) {
                if (pair.first == divisor) {
                    return pair.second;
                }
            }
            return ""; // Should never happen, but good to have a default
        }();
