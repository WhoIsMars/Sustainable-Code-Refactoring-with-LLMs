...
namespace list_ops {
    ...
    std::vector<int> foldl(const std::vector<int>& list, int initial, std::function<int(int, int)> func) {
    ...
    std::vector<int> foldr(const std::vector<int>& list, int initial, std::function<int(int, int)> func) {
        int accumulator = initial;
        for (auto it = list.rbegin(); it != list.rend(); ++it) {
            accumulator = func(*it, accumulator);
        }
        return {accumulator};
    }
