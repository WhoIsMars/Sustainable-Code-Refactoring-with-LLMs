...
std::string beer_song::verse(int verse_num) {
  ...
  std::string num_str = std::to_string(verse_num);
  std::string bottle_word = (verse_num == 1) ? " bottle" : " bottles";
  ...
  std::string next_num_str = std::to_string(verse_num - 1);
  std::string next_bottle_word = (verse_num == 2) ? " bottle" : " bottles";
...
std::string beer_song::sing(int start, int end) {
  std::string lyrics;
  lyrics.reserve((start - end + 1) * 150);
