...
phone_number::phone_number::phone_number(const string &number) {
    ...
    const auto plength = pnumber.length();
    if (plength < 10) throw std::domain_error{"Less than 10 digits"};
    if (plength > 11) throw std::domain_error{"More than 11 digits"};
    
    if (plength == 11) {
        if (pnumber[0] != '1') throw std::domain_error{"Should start with 1"};
        pnumber.erase(0, 1);
    }
    
    if (pnumber[0] <= '1') throw std::domain_error{"Invalid Area-Code"};
    if (pnumber[3] <= '1') throw std::domain_error{"Invalid Exchange-Code"};
