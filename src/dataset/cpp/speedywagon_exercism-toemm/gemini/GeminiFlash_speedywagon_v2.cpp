...
namespace speedywagon {
    ...
    bool alarm_control(pillar_men_sensor* sensor_ptr) {
        return connection_check(sensor_ptr) && sensor_ptr->activity;
    }
    ...
    int uv_light_heuristic(std::vector<int>* data_array) {
        if (data_array->empty()) return 0; // Avoid division by zero and unnecessary computation
    ...
    bool uv_alarm(pillar_men_sensor* sensor_ptr) {
        return connection_check(sensor_ptr) && (uv_light_heuristic(&sensor_ptr->data) > sensor_ptr->activity);
    }
