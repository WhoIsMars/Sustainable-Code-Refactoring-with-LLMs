...
namespace nth_prime {
	...
	int nth(int n) {
		...
		primes.reserve(n); // Reserve space for n primes to avoid repeated allocations
		primes.push_back(2); // Start with the first prime number

		for (int candidate = 3; primes.size() < static_cast<size_t>(n); candidate += 2) { // Skip even numbers
			bool is_prime = true;
