...
measure_result measure(int bucket1_capacity, int bucket2_capacity,
    ...
    while (!queue.empty()) {
        ...
        if (current.a != bucket1_capacity) {
            next_states.push_back({bucket1_capacity, current.b, current.steps + 1});
        }
        if (current.b != bucket2_capacity) {
            next_states.push_back({current.a, bucket2_capacity, current.steps + 1});
        }
        ...
        // Empty buckets
        if (current.a > 0) {
            next_states.push_back({0, current.b, current.steps + 1});
        }
        if (current.b > 0) {
            next_states.push_back({current.a, 0, current.steps + 1});
        }

        for (const auto& next : next_states) {
            if (is_valid_state(next, bucket1_capacity, bucket2_capacity, start_bucket)) {
                queue.push(next);
            }
        }
    }
