...
string verse(int num) {
    ...
    // iteratate over swallows
    for (int j=num; j > 0; j--) {
        out += "She swallowed the " + lyrics[j].first + " to catch the " + lyrics[j-1].first + ((j == 2) ? alt_bird : ".\n");

        // add fly line to end
        if (j == 1) out += lyrics[0].second.second;
    }
    
    // the last verse stands on its own
    if (num == 7) out = lyrics[num].second.first + lyrics[num].second.second;
